...
# ╔═╡ f8143a61-5184-4d08-8aaa-1ad7f49738bd
try 
...
md"""
# EmPoint type from BandPyrometry.jl for blackbody temperture determination
"""
...
* Planck functions from `PlanckFunctions.jl` package
* Optimization of blackbody temperature with and without the `BandPyrometry.EmPoint` type 
...
# ╔═╡ e09a5ed2-8ef7-48af-b4bb-ef1d205024fa
const POINTS_NUMBER = 50

# ╔═╡ baf332fc-a805-4981-b569-70324a879a99
lam = range(lam_left,lam_right,length=POINTS_NUMBER); # full region (as far as EmPoint uses StaticArrays, the length of lam range should not be too high)
...
pretty_table(HTML,hcat(T,PL.∫ibbₗ.(T,λₗ = lam_left,λᵣ=lam_right)), column_labels = ["Temperature", "Fractional power"], top_left_string="Table of fraction of total power within spectral range $(lam_left) ... $(lam_right)")
...
em_point = BP.EmPoint(MVector{POINTS_NUMBER}(Imeas), SVector{POINTS_NUMBER}(collect(lam)))# emissivity point 
...
begin
	...
	hess_values[2] =G[]
	pretty_table(HTML,hcat(["Direct","EmPoint", "AutoDiff"],discr_values,grad_values,hess_values),column_labels = ["calculated using:","discrepancy", "gradient", "hessian"],top_left_string= "Table of discrepancy and its derivatives values, calculated for T=$(TtryK),K" )
end
...
if is_zero_order_run&&is_zero_order_out
		...
		scatter!(lam, PL.ibb(lam, em_sol_zo.T),label="EmPoint solution T=$(em_sol_zo.T)")
...
# ╔═╡ e84c1ee9-9267-4c5c-8a42-5e67fdd96105
md"Em-point solution benchmark:"

# ╔═╡ 8b70cb0a-ce22-4b11-bb75-4240b2b2b36b
if is_zero_order_run && is_zero_order_bench
	begin 
		bm_res[] = @benchmark BP.fit_T!($em_point_zo,  optimizer_name = $zero_order_method[1])
...
if is_first_order_run
	begin 
		...
		em_point_fo = BP.EmPoint(MVector{POINTS_NUMBER}(Imeas), SVector{POINTS_NUMBER}(collect(lam)))# emissivity point 
		em_sol_fo =  BP.fit_T!(em_point_fo,  optimizer_name = first_order_method[1])#solve(em_prob_fo,  first_order_method[2]()) # solution using 
...
is_first_order_run&&is_first_order_out ? em_sol_fo.res.original : nothing
...
if is_first_order_run&&is_first_order_out
		...
		scatter!(lam, PL.ibb(lam, em_sol_fo.T),label="EmPoint solution T=$(em_sol_fo.T)")
		xlabel!("Wavelength, μm")
		ylabel!("Blackbody intensity, a.u.")
end
...
# ╔═╡ 6c1937c8-3d5e-433f-8a60-c85dafb73fea
md"Direct solution benchmark:"
...
# ╔═╡ ada13ad2-84e1-4070-963b-f89c589766ea
md"Em-point solution benchmark:"

# ╔═╡ d76e765f-ff56-4c7e-b61e-0b49e88f558a
if is_first_order_run && is_first_order_bench
	begin 
		bm_res[] = @benchmark  BP.fit_T!($em_point_fo,  optimizer_name = $first_order_method[1])
		t2[]=mean(bm_res[].times)
		bm_res[]
	end
end
...
if is_second_order_run
		...
		# optimization problem with Empoint implementation
		em_point_so =BP.EmPoint(MVector{POINTS_NUMBER}(Imeas), SVector{POINTS_NUMBER}(collect(lam)))		
		em_sol_so = BP.fit_T!(em_point_so,  optimizer_name = second_order_method[1]) # solution using 
...
is_second_order_run&&is_second_order_out ? em_sol_so.res.original : nothing
...
if is_second_order_run&&is_second_order_out
	begin
		...
		title!("Second order method $(second_order_method[1])")
		plot!(lam, PL.ibb(lam, sol_so.u[]),label="Direct solution T=$(sol_so.u[])")
		scatter!(lam, PL.ibb(lam, em_sol_so.T),label="EmPoint solution T=$(em_sol_so.T)")
		xlabel!("Wavelength, μm")
		ylabel!("Blackbody intensity, a.u.")
	end
end
...
if is_second_order_run&&is_second_order_bench
	begin 
		bm_res[] = @benchmark BP.fit_T!(em_point_so,  optimizer_name = second_order_method[1])
		t2[]=mean(bm_res[].times)
		bm_res[]
	end
end
...
if is_zero_order_bench || is_first_order_bench || is_second_order_bench
	...
	pretty_table(HTML,hcat(optims_vec,t_dir_vec,t_em_vec),column_labels = ["Optimizer", "time Direct, ms", "time EmPoint, ms"])
end
...
function plot_bencnhmark(benchmark_measures)
	...
	ylims!(0,1.1*maximum(bars_values))
	return ppp
end;
...
begin 
	...
	bench_plot = plot_bencnhmark(selected_benchs)
end
